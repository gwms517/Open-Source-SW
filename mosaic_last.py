...
ans=input("Mosaic or pictures: ")   #Mosaic/mosaic/MOSAIC/1이면 모자이크 pictures/Pictures/PICTURES2이면 사진

if ans == '1' or ans == 'Mosaic' or ans == 'mosaic' or ans == 'MOSAIC':
    while(True):
        ret, frame = cap.read() 
...
elif ans == '2' or ans == 'pictures' or ans == 'Pictures' or ans == 'PICTURES':
    ...
    while(True):
        ...
        if len(faces):
            for (x,y,w,h) in faces:
                """ 탐지된 얼굴 영역을 이미지로 바꾸는 코드"""
                # 탐지된 얼굴 영역에 맞도록 이미지의 높이와 너비를 바꿔줌.
                t = cv2.resize(mosaic_img, dsize=(h, w), interpolation= cv2.INTER_LINEAR)
                frame[y:y+h, x:x+w] = t # 탐지된 얼굴 영역을 이미지로 모자이크 처리

                """ 사람의 어깨선과 머리선이 이미지와 딱 맞게 이어지도록 구현한 코드
                    개인별 차이 있음... """
